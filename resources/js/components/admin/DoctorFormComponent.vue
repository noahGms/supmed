            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              ...
              <div class="mt-2">
                <form class="flex flex-col">
                  <div class="flex flex-col pt-4" v-if="create">
                  ...
                  <div class="flex flex-col pt-4">
                    <label for="name" class="text-lg">Specialities</label>
                    <div class="flex">
                      <select
                      ...
                      >
                        <option
                          :disabled="checkIfSpecialityAlreadyAdd(speciality)"
                          :value="speciality.id"
                    ...
                    <div class="mt-3">
                      <span
                        ...
                        >{{ speciality.name }}
                        <button
                          @click.prevent="removeSpeciality(speciality)"
                          type="button"
                          class="ml-2"
                        >
                  ...
                  <div class="flex flex-row pt-4">
                    <div class="w-40 mr-4">
                      <label for="street_number" class="text-lg"
                        >Street number</label
                      >
                      <input
                        v-model="doctor.address.street_number"
                        type="number"
                        id="street_number"
                        placeholder="200"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mt-1 leading-tight focus:outline-none focus:shadow-outline"
                    ...
                    <div>
                      <label for="street_name" class="text-lg"
                        >Street name</label
                      >
                      <input
                        v-model="doctor.address.street_name"
                        type="text"
                        id="street_name"
                  ...
                  <div class="flex flex-row pt-4">
                    <div class="w-40 mr-4">
                      <label for="zipcode" class="text-lg">Zipcode</label>
                      <input
...
export default {
  ...
  methods: {
    save () {
      this.doctor.specialities = this.doctor.specialities.map(speciality => {
        return speciality.id
      })
      if (this.create) {
        this.$store.dispatch('newDoctor', this.doctor).then(response => {
          this.doctorListing()
          this.close()
        })
      } else {
        this.$store.dispatch('updateOneDoctor', this.doctor).then(response => {
          this.doctorListing()
          this.close()
        })
      }
    },
    removeSpeciality (specality) {
      this.doctor.specialities = this.doctor.specialities.filter(
        item => item.id !== specality.id
      )
    },
    addSpeciality () {
      this.doctor.specialities.push(
        this.specialities.filter(
          item => item.id === this.specialitiesSelected
        )[0]
      )
    },
    checkIfSpecialityAlreadyAdd (specality) {
      const specialityFiltered = this.doctor.specialities.filter(
        item => item.id === specality.id
      )
    ...
    formValid () {
      if (this.create) {
        return !(
          ...
          !!this.doctor.password_confirmation
        )
      } else {
        return false
      }
